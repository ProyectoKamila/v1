function CNeighborsPanel(){
    ...
    this._init = function(){
        ...
        _oMoneyText = new createjs.Text(_iTotMoney+"$","bold 20px Arial", "#fff");
        ...
        _oBackBut = new CTextButton(CANVAS_WIDTH - (oSprite.width/2) - 10 , 30,oSprite,TEXT_EXIT,"Arial","#fff",14,false);
        _oBackBut.addEventListener(ON_MOUSE_UP, this._onExit, this);
    ...
    this._onExit = function(){
        if (_bFichesOnTable) {
            ...
            for(var i=0;i<_aMcFichesAttached[_iNumberClicked].length;i++){
                s_oGame._onShowBetOnTableFromNeighbors({button:"oBetNeighbors",numbers:aBets,bet_mult:5,bet_win:7,
                                                            value:_aMcFichesAttached[_iNumberClicked][i].getValue(),num_fiches:5});
            }
    ...
    this._onNeighborRelease = function(oParams){
        if(_aFichesAttached.length > 0 && _aMcFichesAttached[oParams.index] === undefined){
            return;
        }
